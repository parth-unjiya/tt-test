...
class FreeResourceReport(models.Model):
    ...
    def _select(self):
        return """
            ...
            COALESCE(SUM(
                ra.allocation_hours /
                NULLIF((
                    SELECT COUNT(*) FROM generate_series(ra.start_date, ra.end_date, interval '1 day') AS d(day)
                    WHERE EXTRACT(DOW FROM d.day) NOT IN (0,6)
                       OR EXISTS (
                            SELECT 1 FROM hr_leave_mandatory_day m
                            WHERE d.day::date BETWEEN m.start_date AND m.end_date
                       )
                ), 0)
    ...
    def _where(self):
        return """
            (
                EXTRACT(DOW FROM gs.day) NOT IN (0, 6)
                OR EXISTS (
                    SELECT 1 FROM hr_leave_mandatory_day m
                    WHERE gs.day::date BETWEEN m.start_date AND m.end_date
                )
            )
        """
