...
class AccountMove(models.Model):
    ...
    approval_ids = fields.One2many(
        "approval.line", "move_id", string="Lines", help="Approval lines"
    )
    document_fully_approved = fields.Boolean(
        string="Document Fully Approved",
        compute="_compute_document_fully_" "approved",
        help="Enable/disable to approve" " document.",
    )
    check_approve_ability = fields.Boolean(
        string="Check Approve Ability",
        compute="_compute_check_approve_" "ability",
        help="Enable/disable to check " "approve ability.",
    )
    is_approved = fields.Boolean(
        string="Is Approved",
        compute="_compute_is_approved",
        help="Enable/disable to compute approved" " or not.",
    )
    ...
    users_approved = fields.Boolean(
        string="Users Approved", help="All the users approved the invoice"
    )
    ...
    def _onchange_partner_id(self):
        ...
        invoice_approval_id = self.env["invoice.approval"].browse(
            self.env.ref(
                "invoice_multi_approval.default_invoice_multi_approval_config"
            ).id
        )
        ...
        approval_managers = self.env["res.users"].search(
            [
                (
                    "groups_id",
                    "in",
                    self.env.ref("invoice_multi_" "approval.group_approve_manager").id,
                )
            ]
        )
            ...
            if approval_managers:
                for manager in approval_managers:
                    if not manager.id in self.approval_ids.approver_id.ids:
                        vals = {"approver_id": manager.id}
                        self.approval_ids |= self.approval_ids.new(vals)
            if (
                invoice_approval_id.approve_customer_invoice
                ...
                invoice_approval_id.approve_vendor_bill
                ...
                invoice_approval_id.approve_customer_credit
                ...
                invoice_approval_id.approve_vendor_credit
    ...
    def _compute_check_approve_ability(self):
        ...
        for rec in self:
            ...
            if self.env.ref(
                "invoice_multi_approval.default_invoice_multi_approval_config"
            ).approve_customer_invoice:
                approvers_list = [
                    approver.approver_id.id for approver in rec.approval_ids
                ]
    ...
    def action_invoice_approve(self):
        ...
        if (
            ...
            invoice_approval_id.approve_vendor_credit and self.move_type == "in_refund"
        ):
    ...
    def _compute_document_fully_approved(self):
        ...
        for rec in self:
            ...
            rec.document_fully_approved = (
                True if length_approval_ids == length_approve_lines else False
            )
            approval_managers = (
                self.env["res.users"]
                .search(
                    [
                        (
                            ...
                            "in",
                            self.env.ref(
                                "invoice_multi_" "approval.group_approve_manager"
                            ).id,
                        )
                    ]
                )
