...
def import_data_to_odoo(models, uid, odoo_model, data):
    ...
    for record in data:
        vals = {
            'title': record['vAttributeName'] if record['vAttributeName'] != None else '',  # Map fields from MySQL to Odoo
            'tt_id': record['iAppraisalAttributeId'] if record['iAppraisalAttributeId'] != None else 0,
            'description': record['txAttributeDescription'] if record['txAttributeDescription'] != None else '',
        ...
        existing_ids = models.execute_kw(odoo_db, uid, odoo_password, odoo_model, 'search', [[('tt_id', '=', record['iAppraisalAttributeId'])]])

        if existing_ids:
            # Update existing record
            models.execute_kw(odoo_db, uid, odoo_password, odoo_model, 'write', [existing_ids, vals])
            print(f"Updated record {record['vAttributeName']}")
        else:
            # Create new record
            models.execute_kw(odoo_db, uid, odoo_password, odoo_model, 'create', [vals])
            print(f"Created new record {record['vAttributeName']}")
...
def main():
    ...
    mysql_table = 'appraisal_attributes'
    odoo_model = 'survey.question'
